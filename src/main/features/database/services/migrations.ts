 ...
 * - It has never been played (no play history)
 */
...
async function isValidGame(gameId: string, doc: any): Promise<boolean> {
  try {
    ...
    // If no monitor path, check if the game has play history
    // A game with play history should not be removed even without monitor path
    ...
    if (record.playTime && record.playTime > 0) {
      log.info(`[Migrations] Game ${gameId} has no monitor path but has play history (playTime: ${record.playTime})`)
      return true
    }
    
    // 3. Has a play status other than 'unplayed'
    if (record.playStatus && record.playStatus !== 'unplayed') {
      log.info(`[Migrations] Game ${gameId} has no monitor path but has play history (status: ${record.playStatus})`)
      return true
    }
    ...
    // Game has no monitor path and no play history - it's invalid
    log.info(`[Migrations] Game ${gameId} has no monitor path and no play history - marking as invalid`)
    return false
